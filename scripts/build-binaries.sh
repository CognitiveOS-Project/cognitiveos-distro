...
LLAMA_CPP_LIB="${LLAMA_CPP_DIR}/build/src"
LLAMA_CPP_INC="${LLAMA_CPP_DIR}/ggml/include"
echo "Building inference (coginfer)..."
cd "${SRC_DIR}/../inference"
CGO_ENABLED=1 CGO_CFLAGS="-I${LLAMA_CPP_INC}" CGO_LDFLAGS="-L${LLAMA_CPP_LIB}" GOOS=linux ${GO} build -tags=cgo -ldflags="-s -w" -o "${BIN_DIR}/cognitiveos-inference" ./cmd/coginfer
echo "  -> cognitiveos-inference built"

echo "Building cograw..."
cd "${SRC_DIR}/../inference"
CGO_ENABLED=1 CGO_CFLAGS="-I${LLAMA_CPP_INC}" CGO_LDFLAGS="-L${LLAMA_CPP_LIB}" GOOS=linux ${GO} build -tags=cgo -ldflags="-s -w" -o "${BIN_DIR}/cograw" ./cmd/cograw
echo "  -> cograw built"
